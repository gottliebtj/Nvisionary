...
const Windows = (props) => {
    ...
    const windows = useGLTF('./windows/scene.gltf');
    const { nodes, scene, materials, animations } = windows;
    ...
    useFrame((state, delta) => {
        mixer = new THREE.AnimationMixer(windows.scene);
        var action = mixer.clipAction(windows.animations[animationIndex]).play();
        action.play();
        //console.log(delta)
    ...
    return (
        <primitive
            object={windows.scene}
            position={[positionZ, 25, -25]}
            scale={1.2}
